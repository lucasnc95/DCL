...
Impl(int& argc, char**& argv)
    ...
    ~Impl() {
        ...
        for (std::size_t i = 0; i < local_devices_.size(); ++i) {
            if (local_devices_[i].kernel_queue != nullptr) {
                clReleaseCommandQueue(local_devices_[i].kernel_queue);
            }
            if (local_devices_[i].transfer_queue != nullptr) {
                clReleaseCommandQueue(local_devices_[i].transfer_queue);
            }
        }

        for (std::size_t i = 0; i < platforms_.size(); ++i) {
            if (platforms_[i].context != nullptr) {
                clReleaseContext(platforms_[i].context);
            }
        }
...
void discover_devices(const DeviceSelection& selection) {
    ...
    check_cl(
        clGetPlatformIDs(n_platforms, platform_ids.data(), nullptr),
    ...
    for (cl_uint p = 0; p < n_platforms; ++p) {
        ...
        for (cl_uint i = 0; i < dev_count; ++i) {
            if (next_local_index >= max_per_rank) {
                break;
            }
            ...
            if (!matches(selection.kind, kind)) {
                continue;
            }
        ...
        for (std::size_t i = 0; i < selected_devices.size(); ++i) {
            ...
            check_cl(
                clGetDeviceInfo(ld.device, CL_DEVICE_NAME, sizeof(name_buf), name_buf, nullptr),
            ...
            check_cl(
                clGetDeviceInfo(ld.device, CL_DEVICE_TYPE, sizeof(dtype), &dtype, nullptr),
                "clGetDeviceInfo(CL_DEVICE_TYPE)"
            );
            ...
            ld.kernel_queue = clCreateCommandQueueWithProperties(
                ld.context,
                ld.device,
                qprops,
                &qerr
            ...
            ld.transfer_queue = clCreateCommandQueueWithProperties(
                ld.context,
                ld.device,
                qprops,
                &qerr
...
#else
            ld.kernel_queue = clCreateCommandQueue(
                ld.context,
                ld.device,
            ...
            ld.transfer_queue = clCreateCommandQueue(
                ld.context,
                ld.device,
                CL_QUEUE_PROFILING_ENABLE,
                &qerr
    ...
    for (int r = 0; r < rank_; ++r) {
        global_base += all_device_counts_[r];
    }
    ...
    BufferHandle create_buffer(const BufferSpec& spec) {
        ...
        for (std::size_t d = 0; d < local_devices_.size(); ++d) {
            ...
            rb.replicas[d] = clCreateBuffer(
                local_devices_[d].context,
                flags,
                spec.bytes,
                const_cast<void*>(spec.host_ptr),
                &err
            );
...
FieldHandle create_field(const FieldSpec& spec) {
    ...
    if (spec.global_elements == 0) {
        throw Error("FieldSpec.global_elements must be > 0");
    }
    if (spec.units_per_element == 0) {
        throw Error("FieldSpec.units_per_element must be > 0");
    }
    if (spec.bytes_per_unit == 0) {
        throw Error("FieldSpec.bytes_per_unit must be > 0");
    }
    ...
    const std::size_t total_bytes =
        spec.global_elements * spec.units_per_element * spec.bytes_per_unit;

    const cl_mem_flags flags = to_opencl_flags(spec.usage, false);

    for (std::size_t d = 0; d < local_devices_.size(); ++d) {
        cl_int err = CL_SUCCESS;
        rf.replicas[d] = clCreateBuffer(
            local_devices_[d].context,
            flags,
            total_bytes,
            nullptr,
    ...
    KernelHandle create_kernel(const KernelSpec& spec) {
        ...
        for (std::size_t d = 0; d < local_devices_.size(); ++d) {
            ...
            cl_kernel k = clCreateKernel(
                rk.programs_per_platform[p],
                spec.entry_point.c_str(),
                &err
            );
...
void execute(const ExecutionStep& step) {
    ...
    for (std::size_t inv = 0; inv < step.invocations.size(); ++inv) {
        ...
        std::unordered_map<int, RegisteredKernel>::iterator kit =
            kernels_.find(ki.binding.kernel.value);
        if (kit == kernels_.end()) {
            throw Error("Unknown kernel in execute()");
        }

        RegisteredKernel& rk = kit->second;
        std::vector<double> elapsed_local(local_devices_.size(), 0.0);

        if (step.halo.width_elements > 0 && !step.halo.fields.empty()) {
            run_interior_phase(rk, ki, elapsed_local, step.halo.width_elements);
            exchange_halo_set(step.halo);
            synchronize_all_local_devices();
            run_border_phase(rk, ki, elapsed_local, step.halo.width_elements);
        } else {
            run_full_phase(rk, ki, elapsed_local);
        }
        ...
        if (step.balance.mode != BalanceMode::off) {
            ...
            if ((iteration_counter_ % static_cast<std::size_t>(every)) == 0) {
                if (!last_output_fields_.empty()) {
                    rebalance(last_output_fields_.front());
                }
            }
        } else {
            ++iteration_counter_;
    ...
    void rebalance(FieldHandle target_field) {
        ...
        check_mpi(
            MPI_Allreduce(local_times.data(), global_times.data(), static_cast<int>(global_times.size()), MPI_DOUBLE, MPI_SUM, comm_),
    ...
    void gather(FieldHandle field, void* host_dst, std::size_t bytes) {
        ...
        if (rank_ == 0) {
            ...
            check_mpi(
                MPI_Reduce(local_image.data(), reduced.data(), static_cast<int>(total_bytes), MPI_BYTE, MPI_BOR, 0, comm_),
                "MPI_Reduce(gather root)"
            );
            std::memcpy(host_dst, reduced.data(), total_bytes);
        } else {
            check_mpi(
                MPI_Reduce(local_image.data(), nullptr, static_cast<int>(total_bytes), MPI_BYTE, MPI_BOR, 0, comm_),
                "MPI_Reduce(gather non-root)"
            );
        }
    }
    ...
    void bind_kernel_args(std::size_t local_device, cl_kernel kernel, const KernelBinding& binding) {
        for (std::size_t i = 0; i < binding.args.size(); ++i) {
            ...
            if (const ScalarArg* sa = std::get_if<ScalarArg>(&arg)) {
                check_cl(
                    clSetKernelArg(kernel, arg_index, sa->bytes.size(), sa->bytes.data()),
            ...
            check_cl(
                clSetKernelArg(kernel, arg_index, sizeof(cl_mem), &mem),
                "clSetKernelArg(mem)"
            );
        }
    }
...
void run_full_phase(
    ...
    for (std::size_t p = 0; p < partitions_.size(); ++p) {
        const DevicePartition& dp = partitions_[p];
        if (dp.owning_rank != rank_ || dp.local_index < 0) {
            continue;
        }
        ...
        check_cl(clFinish(local_devices_[d].kernel_queue), "clFinish(full)");

        cl_ulong t0 = 0;
        cl_ulong t1 = 0;

        cl_int e0 = clGetEventProfilingInfo(
            kernel_ev,
        ...
        cl_int e1 = clGetEventProfilingInfo(
            kernel_ev,
        ...
        if (kernel_ev != nullptr) {
            clReleaseEvent(kernel_ev);
        }
    }
}

void run_interior_phase(
    RegisteredKernel& rk,
    const KernelInvocation& ki,
    std::vector<double>& elapsed_local,
    std::size_t halo
    ...
    for (std::size_t p = 0; p < partitions_.size(); ++p) {
        ...
        check_cl(clFinish(local_devices_[d].kernel_queue), "clFinish(interior)");
...
void run_border_phase(
    ...
    for (std::size_t p = 0; p < partitions_.size(); ++p) {
        ...
        if (left_count > 0) {
            ...
            check_cl(clFinish(local_devices_[d].kernel_queue), "clFinish(border-left)");
        ...
        if (right_count > 0 && dp.element_count > left_count) {
            const std::size_t gwo =
                dp.global_offset + dp.element_count - right_count + ki.geometry.global_offset;
            ...
            check_cl(clFinish(local_devices_[d].kernel_queue), "clFinish(border-right)");
...
static double elapsed_between(cl_event start_ev, cl_event end_ev) {
    if (start_ev == nullptr || end_ev == nullptr) {
        return 0.0;
    }

    cl_int err = clWaitForEvents(1, &start_ev);
    if (err != CL_SUCCESS) {
        return 0.0;
    }

    err = clWaitForEvents(1, &end_ev);
    if (err != CL_SUCCESS) {
        return 0.0;
    }
    ...
    if (e0 != CL_SUCCESS || e1 != CL_SUCCESS) {
        return 0.0;
    }
...
void exchange_halo_set(const HaloSpec& hs) {
    ...
    void exchange_halos_for_field(FieldHandle fh, std::size_t halo) {
        if (halo == 0 || partitions_.size() < 2) return;

        std::unordered_map<int, RegisteredField>::iterator it = fields_.find(fh.value);
        if (it == fields_.end()) throw Error("Unknown field in halo exchange");
        ...
        for (std::size_t b = 0; b + 1 < partitions_.size(); ++b) {
            ...
            if (left.owning_rank == rank_ && right.owning_rank == rank_) {
                ...
                if (left.owning_rank == rank_) {
                    const int tag = 7000 + static_cast<int>(b) + fh.value * 1000;
                    check_mpi(
                        MPI_Send(send_right.data(), static_cast<int>(halo_bytes), MPI_BYTE, right.owning_rank, tag, comm_),
                        "MPI_Send(halo left->right)"
                    );
                }
                if (right.owning_rank == rank_) {
                    const int tag = 7000 + static_cast<int>(b) + fh.value * 1000;
                    check_mpi(
                        MPI_Recv(recv_left.data(), static_cast<int>(halo_bytes), MPI_BYTE, left.owning_rank, tag, comm_, MPI_STATUS_IGNORE),
                        "MPI_Recv(halo left->right)"
                    );
                    const std::size_t dr = static_cast<std::size_t>(right.local_index);
                    check_cl(
                        clEnqueueWriteBuffer(
            ...
            if (left.owning_rank == rank_ && right.owning_rank == rank_) {
                ...
                if (right.owning_rank == rank_) {
                    const int tag = 8000 + static_cast<int>(b) + fh.value * 1000;
                    check_mpi(
                        MPI_Send(send_left.data(), static_cast<int>(halo_bytes), MPI_BYTE, left.owning_rank, tag, comm_),
                        "MPI_Send(halo right->left)"
                    );
                }
                if (left.owning_rank == rank_) {
                    const int tag = 8000 + static_cast<int>(b) + fh.value * 1000;
                    check_mpi(
                        MPI_Recv(recv_right.data(), static_cast<int>(halo_bytes), MPI_BYTE, right.owning_rank, tag, comm_, MPI_STATUS_IGNORE),
                        "MPI_Recv(halo right->left)"
                    );
                    const std::size_t dl = static_cast<std::size_t>(left.local_index);
                    check_cl(
                        clEnqueueWriteBuffer(
    ...
    void synchronize_all_local_devices() {
        for (std::size_t d = 0; d < local_devices_.size(); ++d) {
            check_cl(clFinish(local_devices_[d].kernel_queue), "clFinish(kernel queue)");
            check_cl(clFinish(local_devices_[d].transfer_queue), "clFinish(transfer queue)");
        }
    }
    ...
    void redistribute_field_intersection(
        FieldHandle fh,
        ...
        for (std::size_t src = 0; src < old_parts.size(); ++src) {
            for (std::size_t dst = 0; dst < new_parts.size(); ++dst) {
                ...
                if (rank_src == rank_ && rank_dst == rank_) {
                    ...
                    if (rank_src == rank_) {
                        check_mpi(
                            MPI_Send(tmp.data() + byte_off, static_cast<int>(byte_len), MPI_BYTE, rank_dst, tag, comm_),
                            "MPI_Send(redistribute)"
                        );
                    }
                    ...
                    if (rank_dst == rank_ && new_parts[dst].local_index >= 0) {
                        const std::size_t ddst = static_cast<std::size_t>(new_parts[dst].local_index);
                        check_mpi(
                            MPI_Recv(tmp.data() + byte_off, static_cast<int>(byte_len), MPI_BYTE, rank_src, tag, comm_, MPI_STATUS_IGNORE),
                            "MPI_Recv(redistribute)"
                        );
                        check_cl(
                            clEnqueueWriteBuffer(
                                local_devices_[ddst].transfer_queue,
